...
from helpers import model_metadata_response, get_model_metadata
...
def colorize_image(file, SHAPE=256):
  original_frame=convert_to_opencv(file)

  (h_orig, w_orig) = original_frame.shape[:2]
  
  grpc_address= os.getenv("grpc_address", '127.0.0.1')
  ...
  for input_name in input_metadata:
    request.inputs[input_name].CopyFrom(make_tensor_proto(np.zeros(input_metadata[input_name]['shape']), dtype=tf.dtypes.float32))

  request.inputs['data_l'].CopyFrom(make_tensor_proto(img_l_rs, shape=(1,1,256,256)))
